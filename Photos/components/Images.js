...
function Images() {
    ...
    const deleteSelectedImages = (id) => {
        ...
        db.transaction(tx => {
            tx.executeSql('DELETE FROM photocollection WHERE id = ?', [id], (_, result) => {
                        ...
                        <View key={index} style={{justifyContent: 'center', alignContent:'center'}}>
                            ...
                            <View style={{ flexDirection: 'row', alignItems: 'center', justifyContent: 'space-around', marginTop: -5 }}>
                                <TouchableOpacity onPress={() => deleteSelectedImages(item.id)}>
                                    <MaterialCommunityIcons name='delete' style={styles.iconDelete} />
                                </TouchableOpacity>
                                <TouchableOpacity onPress={() => openImageInfo(item.id)}>
                                    <MaterialCommunityIcons name='information' style={styles.iconInfo} />
                                </TouchableOpacity>
                            </View>
                        </View>
                    ))}
                ...
                <View style={{ flex: 1 }}>
                    <View style={{ flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', marginTop: 80 }}>
                        <TouchableOpacity onPress={closeImageModal}>
                            <MaterialCommunityIcons name='close' style={styles.iconClose} size='30' />
                        </TouchableOpacity>
                    </View>
                    <Image source={{ uri: selectedImage }} style={{ flex: 1, marginBottom: 70 }} resizeMode="contain" />
                </View>
            </Modal>
            <Modal
                animationType="slide"
